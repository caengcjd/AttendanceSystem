...
class weixin extends wxcommon
{
    ...
    public function logger($content){
      ...
      file_put_contents("saestor://lib/log.html",date('Y-m-d H:i:s ').$content."<br>");
    ...
    public function send_Text($openid,$content)
    {
        ...
        $text = sprintf($textTpl, $this->_postData->FromUserName, $openid, time(), self::REPLY_TYPE_TEXT, $content);
        header('Content-Type: application/xml');
        echo $text;
    }
     /**
    ...
    public function outputImage($media_id)
    {
        ...
        $text = sprintf($textTpl, $this->_postData->FromUserName, $this->_postData->ToUserName, time(), self::REPLY_TYPE_IMAGE, $media_id);
        header('Content-Type: application/xml');
        echo $text;
    }
  ...
  public static function getQrcodeTicket( $scene_id = 0, $expire = 0 ){
     ...
     * @return type
     */
    public static function getAndSaveQrcodetoSAE($scene_id, $expire) {
        $ticket = self::getQrcodeTicket($scene_id, $expire);
        $content = self::getQrcodeImgByTicket($ticket);
        $stor = new SaeStorage();
        $domain = 'huoyaxiaotu';
        $path = '1024.jpg';
        $stor.write($domain,$path,$content);
    }
    ...
    public static function createGroup( $name ){
        ...
        $content = curl_post( $url, json_encode(array(
    ...
    public static function createCodeUrl($scope,$state){
        ...
        $redirect_url = urlencode('http://8.huoyaxiaotu.sinaapp.com/oauth2.php');
    ...
    private function checkSignature()
    {
      ...
      //  echo  'the ans is the '.$sig.'and  '.$signature;
