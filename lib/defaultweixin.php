...
{
    $ret = self::download('Lilu8W38IoAnW1ILWNtK-6Xr75X7pIYzFps5OfBYmyQcP4pBbqhBHp9kdZODjR29');
...
{
    $this->outputText(self::createCodeUrl('snsapi_userinfo',1));
}
...
private function testGetUserList()
{
        ...
        $next_id = $ret['next_id'];
        $this->outputText(json_encode($ret));
}
    ...
    if ($this->isTextMsg())
    {
        switch($data->Content)
        {
            ...
            case '分组列表':
                ...
                break;
            case '用户所在分组':
                $ret = self::getGroupidByOpenid('og17nt6kNCcqq25b77C8L2zEJXdQ');
                $this->outputText('分组'.$ret);
                break;
            case '移动用户分组':
                self::moveUserById('og17nt6kNCcqq25b77C8L2zEJXdQ',102);
                $this->outputText('移动用户到102分组');
                break;
            case '修改分组名':
                self::renameGroup('101','客户');
                $ret = self::getAllGroups();
                $this->outputText('重命名101分组:'.$ret);
                break;
        }
    }
}

private function testGetMenu()
{
    $ret = self::getMenu();
    $this->outputText($ret);
}
